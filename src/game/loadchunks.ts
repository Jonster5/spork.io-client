...
import { Canvas, SocketMessageEvent, Sprite, Transform, checkTimer, encodeString, getSocket, setTimer, stitch } from "raxis-plugins";
...
function enableMap(ecs: ECS) {
    ecs.getEventReader(LoadMinimapEvent).get().forEach((event) => {
    ...
    for (let i = 0; i < 11; i++) {
        for (let j = 0; j < 11; j++) {
            ...
            for (let [loadedChunk] of loadedChunks) {
                if (loadedChunk.position.x == gridPosition.x + j-5 && loadedChunk.position.y == gridPosition.y + i-5) {
                    unloaded = false
                    break;
                }
            }
...
function loadChunks(ecs: ECS) {
    ...
    ecs.getEventReader(SocketMessageEvent).get().forEach((event) => {
        ...
        const biomes = new Uint8Array(event.body.slice(0,event.body.byteLength/5))
        const chunks = new Int16Array(event.body.slice(event.body.byteLength/5))
        const [map] = ecs.query([LoadedMap], With(Player)).single()
        const loadedChunks = ecs.query([Chunk]).results()
        ...
        for (let i = 0; i < chunks.length; i += 2) {
            ...
            if (!chunkOverlap) {
                ...
                const chunk = ecs.spawn(new Chunk(new Vec2(chunks[i], chunks[i+1]), 0), new Sprite('rectangle', color), new Transform(new Vec2(500,500), new Vec2(chunks[i]*500, chunks[i+1]*500)))
