...
export class LoadMinimapEvent extends ECSEvent{
    constructor(
        public minimapData: ArrayBuffer
    ) {
        super()
    }
}
...
function loadMinimap(ecs: ECS) {
    ecs.getEventReader(LoadMinimapEvent).get().forEach((event) => {
        ...
        ecs.spawn(new Minimap(), new Transform(new Vec2(400,400)), new Sprite('image', [canvas], 1))
        ecs.spawn(new PlayerMapIcon(), new Transform(new Vec2(5,5)), new Sprite('rectangle', 'black', 2))
